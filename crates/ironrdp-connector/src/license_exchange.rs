...
mod tests {
    ...
    /// directly rather than destructured out of the state. This drive never
    /// leaves `NewLicenseRequest`, so it pins only that the counter climbs
    /// monotonically and the state stays put within one state — it cannot
    /// distinguish that from a per-state counter, since both would climb the
    /// same way here. (The counter is in fact cumulative across all three
    /// waiting states, not reset per state, but that is benign: it only ever
    /// feeds the `SKIP_WARN_THRESHOLD` check in `log_skipped_non_license_pdu`
    /// that picks `debug!` vs. `warn!`, and that log line separately carries
    /// `state` — no functional path reads `skip_count`.)
    #[test]
