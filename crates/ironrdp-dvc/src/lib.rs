...
/// A batch of outgoing DRDYNVC messages associated with one dynamic channel.
#[derive(Debug)]
...
impl DvcMessageBatch {
    pub fn new(channel_id: DynamicChannelId, messages: Vec<SvcMessage>) -> Self {
        Self { channel_id, messages }
    }

    /// Creates a batch after verifying that every message carries `channel_id`.
    ...
    pub fn into_parts(self) -> (DynamicChannelId, Vec<SvcMessage>) {
        (self.channel_id, self.messages)
    }
