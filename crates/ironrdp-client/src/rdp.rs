...
async fn connect_direct(
    ...
    #[cfg(feature = "udp")]
    let udp_peer = enable_udp.then_some(udp_peer);
    #[cfg(not(feature = "udp"))]
    let udp_peer = None;
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static,
{
    ...
    if let Some(udp_peer) = udp_peer {
        let udp_state = Arc::new(Mutex::new(UdpBootstrapState::default()));
        let state_for_handler = Arc::clone(&udp_state);
        ...
        let config_for_handler = udp_config.clone();
                ...
                async move {
                    {
                        ...
                        if state.attempted_protocols.contains(&request.requested_protocol) {
                            return Ok(ironrdp_connector::MultitransportResult::Failure(
                                ironrdp_pdu::rdp::multitransport::MultitransportResponsePdu::E_ABORT,
                            ));
                        }
                        state.attempted_protocols.push(request.requested_protocol);
                    }
                    match bootstrap_udp_transport(request, udp_config).await {
                        Ok(transport) => {
                            if soft_sync {
                                state.lock().await.transport = Some(transport);
                            }
        ...
        .await?;
        let mut state = udp_state.lock().await;
        let udp_tunnel = UdpTunnel {
            transport: state.transport.take(),
            bootstrap: Some(udp_config),
            attempted_protocols: core::mem::take(&mut state.attempted_protocols),
        };
        drop(state);
            ...
            tokio::select! {
                ...
                input_event = input_event_receiver.recv() => {
                    ...
                    RdpInputEvent::FastPath(events) => {
                        ...
                        ActiveSessionIteration::outputs(active_stage.process_fastpath_input(&mut image, &events)?)
                    }
